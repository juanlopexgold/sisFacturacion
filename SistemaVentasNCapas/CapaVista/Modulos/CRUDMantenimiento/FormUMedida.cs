...
{
    public partial class FormUMedida : Form
    {
        ...
        {
            this.dataGridListado.DataSource = CN_UMedida.Mostrar();
        ...
        {
            this.dataGridListado.DataSource = CN_UMedida.Buscar_nombre(this.textBusqueda.Text);
            this.OcultarColumnas();
            labelTotal.Text = "Total de registros: " + Convert.ToString(dataGridListado.Rows.Count);
        }

        private void FormUMedida_Load(object sender, EventArgs e)
        {
            this.MostraColumnas();
            this.Habilitar(false);
        ...
        private void iconRegistrar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    if (this.IsNuevo)
                    {
                        repuesta = CN_UMedida.Insertar(this.textNombre.Text.Trim().ToUpper(),
                            Convert.ToInt32(((ObcionesCombo)comboEstado.SelectedItem).valor) == 1 ? true : false);
                    }
                    else
                    {
                        repuesta = CN_UMedida.Actualizar(Convert.ToInt32(this.textID.Text),
        ...
        {
            this.textID.Text = Convert.ToString(this.dataGridListado.CurrentRow.Cells["ID_UMEDIDA"].Value);
            this.textNombre.Text = Convert.ToString(this.dataGridListado.CurrentRow.Cells["UNIDAD_MEDIDA"].Value);
                ...
                if (Obcion == DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow row in dataGridListado.Rows)
                    {
                        if (Convert.ToBoolean(row.Cells[0].Value))
                        {
                            ...
                            respu = CN_UMedida.Eliminar(Convert.ToInt32(codigo));
