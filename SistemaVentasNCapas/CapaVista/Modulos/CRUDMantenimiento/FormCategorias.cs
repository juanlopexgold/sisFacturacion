...
namespace CapaVista.Modulos.CRUDMantenimiento
{
    public partial class FormCategorias : Form
    {
        ...
        {
            this.dataGridListado.DataSource = CN_Categorias.Mostrar();
        ...
        {
            this.dataGridListado.DataSource = CN_Categorias.Buscar_nombre(this.textBusqueda.Text);
            this.OcultarColumnas();
            labelTotal.Text = "Total de registros: " + Convert.ToString(dataGridListado.Rows.Count);
        }
        ...
        private void iconRegistrar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    if (this.IsNuevo)
                    {
                        repuesta = CN_Categorias.Insertar(this.textNombre.Text.Trim().ToUpper(),
                            Convert.ToInt32(((ObcionesCombo)comboEstado.SelectedItem).valor) == 1 ? true : false);
                    }
                    else
                    {
                        repuesta = CN_Categorias.Actualizar(Convert.ToInt32(this.textID.Text),
        ...
        {
            this.textID.Text = Convert.ToString(this.dataGridListado.CurrentRow.Cells["ID_CATEGORIA"].Value);
            this.textNombre.Text = Convert.ToString(this.dataGridListado.CurrentRow.Cells["NOMBRE_CATEGORIA"].Value);
                ...
                if (Obcion == DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow row in dataGridListado.Rows)
                    {
                        if (Convert.ToBoolean(row.Cells[0].Value))
                        {
                            ...
                            respu = CN_Categorias.Eliminar(Convert.ToInt32(codigo));
