...
namespace CapaVista.Modulos.CRUDMantenimiento
{
    public partial class FormMarcas : Form
    {
        ...
        {
            this.dataGridListado.DataSource = CN_Marcas.Mostrar();
        ...
        {
            this.dataGridListado.DataSource = CN_Marcas.Buscar_nombre(this.textBusqueda.Text);
            this.OcultarColumnas();
            labelTotal.Text = "Total de registros: " + Convert.ToString(dataGridListado.Rows.Count);
        }
        ...
        private void iconRegistrar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    if (this.IsNuevo)
                    {
                        repuesta = CN_Marcas.Insertar(this.textNombre.Text.Trim().ToUpper(),
                            Convert.ToInt32(((ObcionesCombo)comboEstado.SelectedItem).valor) == 1 ? true : false);
                    }
                    else
                    {
                        repuesta = CN_Marcas.Actualizar(Convert.ToInt32(this.textID.Text),
        ...
        {
            this.textID.Text = Convert.ToString(this.dataGridListado.CurrentRow.Cells["ID_MARCA"].Value);
            this.textNombre.Text = Convert.ToString(this.dataGridListado.CurrentRow.Cells["NOMBRE_MARCA"].Value);
                ...
                if (Obcion == DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow row in dataGridListado.Rows)
                    {
                        if (Convert.ToBoolean(row.Cells[0].Value))
                        {
                            ...
                            respu = CN_Marcas.Eliminar(Convert.ToInt32(codigo));
